    ...
    namespace Biblio_App.Services
    {
        ...
        public class TokenHandler : DelegatingHandler
        {
            ...
            protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
            {
                ...
                try
                {
                    // Attempt request; this may throw HttpRequestException when the host is unreachable
                    response = await base.SendAsync(request, cancellationToken);
                }
                catch (HttpRequestException)
                {
                    // Map network-level failures to a ServiceUnavailable response so callers can handle it gracefully
                    return new HttpResponseMessage(HttpStatusCode.ServiceUnavailable)
                    {
                        RequestMessage = request,
                        ReasonPhrase = "Unable to connect to API (network error)"
                    };
                }
